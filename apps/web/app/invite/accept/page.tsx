...
import { Card, Tag, Button, Alert, Spin, App } from "antd";
import {
  MailOutlined,
  SafetyOutlined,
  CloseCircleOutlined,
...
function InviteAcceptForm() {
  ...
  const [invitation, setInvitation] =
    useState<invitationService.OrganizationInvitation | null>(null);
  const [loading, setLoading] = useState(true);
  const [actionLoading, setActionLoading] = useState(false);
  const [errorState, setErrorState] = useState<{
    type: "EXPIRED" | "ACCEPTED" | "DECLINED" | "INVALID" | "UNKNOWN";
    message: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (!token) {
      setErrorState({
        type: "INVALID",
        message: "Lời mời không hợp lệ hoặc thiếu mã xác nhận.",
      });
      setLoading(false);
      return;
    }
    ...
    const fetchInvitation = async () => {
      try {
        ...
        if (res.success) {
          ...
          if (invite.status === "EXPIRED") {
            setErrorState({
              type: "EXPIRED",
              message:
                "Lời mời đã hết hạn, vui lòng liên hệ người mời để gửi lại.",
            });
            ...
            setErrorState({
              type: "INVALID",
              message: "Lời mời này đã bị thu hồi.",
            });
          }
        }
      } catch (err: any) {
        ...
        if (err.status === 404) {
          setErrorState({
            type: "INVALID",
            message: "Lời mời không tồn tại hoặc không hợp lệ.",
          });
          ...
          setErrorState({
            type: "UNKNOWN",
            message: err.message || "Có lỗi xảy ra khi tải thông tin lời mời.",
          });
        }
      } finally {
        setLoading(false);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const targetOrgSlug =
          res.member.organization?.slug || res.member.organizationId;
  ...
  if (errorState || !token) {
    const errorMessage =
      errorState?.message || "Lời mời không hợp lệ hoặc thiếu mã xác nhận.";

    return (
      <Card className="surface-card w-full max-w-[480px] p-8">
        <div className="text-center">
          ...
          <Button
            ...
            className="action-primary focus-ring min-h-11 w-full border-none font-bold"
          >
            Quay về Dashboard
          </Button>
        </div>
  ...
  return (
    <Card className="w-full max-w-[480px] p-8 rounded-2xl shadow-xl border border-gray-100/50 bg-white">
      <div className="flex flex-col items-center text-center">
        ...
        <div className="mb-4 w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center">
          <MailOutlined className="text-gray-700 text-xl" />
        </div>

        <span className="text-xs uppercase font-bold tracking-widest text-gray-600 mb-1">
          Lời mời tham gia
        </span>
        ...
        <p className="text-sm text-gray-500 mb-6">
          ...
          với vai trò:{" "}
          <Tag
            color={ROLE_COLORS[invitation.invitedRole] || "default"}
        ...
        {!user ? (
          <div className="w-full space-y-3">
            ...
            <Button
              ...
              className="w-full min-h-11 action-primary border-none font-bold focus-ring motion-reduce:transform-none"
            >
              Đăng nhập tài khoản
            </Button>
            <Button
              ...
              className="w-full min-h-11 action-secondary font-semibold focus-ring"
            >
          ...
          <div className="w-full space-y-4">
            ...
            <Button
              ...
              className="w-full h-11 rounded-xl font-bold active:scale-[0.98] transition-colors"
            >
              Đăng xuất & Đăng nhập email khác
            </Button>
            <Button
              onClick={() => router.push("/dashboard")}
              className="w-full h-11 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-xl transition-colors font-semibold"
            >
          ...
          <div className="w-full space-y-3">
            <Button
              ...
              loading={actionLoading}
              className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-md border-none font-bold active:scale-[0.98] transition-colors"
            >
              Chấp nhận tham gia
            </Button>
            <Button
