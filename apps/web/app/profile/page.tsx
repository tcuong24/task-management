...
export default function StandaloneProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "team" | "people">(
    "profile",
  );
  ...
  const currentUserRole = useMemo(() => {
    ...
    const m = user.memberships.find(
      (mem) => mem.organization.id === selectedOrgId,
    );
  ...
  const teamColumns = [
    {
      ...
      render: (_: any, record: TeamRowItem) => {
        const isSelf =
          authUser?.id === record.userId || user?.id === record.userId;
    ...
    <div className="min-h-screen bg-white text-left font-sans flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200/80 px-6 py-3 sticky top-0 z-30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            href="/dashboard"
            className="font-brand text-xl font-bold text-gray-700"
          >
            TaskFlow
          </Link>
        </div>
        ...
        <aside className="w-full md:w-64 flex flex-col gap-2 shrink-0 pr-2 border-r border-gray-100">
          <div className="flex items-center gap-3 mb-2">
            <Button
              ...
              className="rounded-xl border-none shadow-none text-gray-600 hover:text-gray-700 hover:border-gray-300 text-xs font-semibold"
            >
              Quay lại Dashboard
            </Button>
          </div>
          ...
          <nav className="flex flex-col gap-1">
            ...
            <button
              ...
              className={`relative flex items-center gap-3.5 px-4 py-1.5 rounded-xl text-sm font-semibold transition-colors cursor-pointer text-left ${
                activeTab === "profile"
                  ? "bg-gray-50/80 text-gray-800 font-bold"
                  : "bg-white text-gray-600 hover:bg-gray-100/70 hover:text-gray-900 font-medium"
              }`}
            >
              {activeTab === "profile" && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-4 bg-blue-600 rounded-lg" />
              )}
              <UserOutlined
                className={`text-base ${activeTab === "profile" ? "text-gray-700" : "text-gray-500"}`}
              />
            ...
            >
              ...
              <TeamOutlined
                className={`text-base ${activeTab === "team" ? "text-gray-700" : "text-gray-500"}`}
              />
              <span>Đội ngũ</span>
            </button>
            ...
            >
              ...
              <UserOutlined
                className={`text-base ${activeTab === "people" ? "text-gray-700" : "text-gray-500"}`}
              />
              <span>Mọi người</span>
            </button>
          </nav>
                    ...
                    <div className="p-4 rounded-2xl bg-white border border-gray-200/80 shadow-sm text-sm text-gray-600 leading-relaxed italic hover:border-gray-300 transition-colors cursor-pointer">
                      {user.fullName ? (
                        <span className="not-italic text-gray-800 font-medium">
                          Tôi là {user.fullName} (@{user.username}). Tôi ưu tiên
                          cập nhật tiến độ công việc trên ứng dụng TaskFlow.
                        </span>
                      ) : (
                        <span>
                          e.g. I prefer async updates in TaskFlow. For urgent
                          questions, ping me between 9am-5pm.
                        </span>
                      )}
                    </div>
                  </div>
                      ...
                      <div className="flex flex-col divide-y divide-gray-100 border border-gray-200/80 rounded-2xl p-2 bg-white shadow-sm">
                        {recentTasks.map((task) => (
                          <div
                            ...
                            onClick={() => {
                              const slug =
                                user.memberships?.[0]?.organization?.slug;
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-200/80 shadow-sm flex flex-col gap-6 text-left">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-gray-100">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs font-semibold text-gray-400">
                    {teamTableData.length} thành viên · Vai trò của bạn:{" "}
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-200/80 shadow-sm flex flex-col gap-6 text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredUsers.map((u) => (
                  <div
                    key={u.id}
                    className="p-4 rounded-2xl border border-gray-200/80 bg-white hover:border-gray-200 hover:shadow-sm transition-colors flex items-start gap-3.5"
                  >
                    <Avatar
                      ...
                      {u.memberships && u.memberships.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {u.memberships.slice(0, 2).map((m) => (
                            <span
                              key={m.organization.id}
                              className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-gray-50 text-gray-800 border border-gray-100"
                            >
                              {m.organization.name}
                            </span>
                          ))}
