...
export default function OrgProjectsPage() {
  ...
  const handleCreateFinish = async (values: { name: string; key?: string; description?: string }) => {
    if (!orgId) return;
    try {
      setCreating(true);
  ...
  const canCreateProject = userRole && hasPermission(userRole, 'project:create');
  ...
  return (
    <div className="p-3 md:p-4 flex flex-col gap-6  mx-auto w-full text-left">
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center">
          ...
          <p className="text-sm text-gray-400 max-w-md mt-1 mb-4">
            Tổ chức này chưa tạo dự án nào. Hãy tạo dự án đầu tiên để bắt đầu quản lý công việc!
          </p>
          {canCreateProject && (
            <Button
              type="primary"
          ...
          {projects.map((p) => {
            ...
            return (
              <Card
                ...
                className="border border-gray-100 shadow-sm hover:shadow-md rounded-2xl transition-all overflow-hidden flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-extrabold text-xs px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-100">
                      KEY: {p.key}
                    </span>
                    <Tag color={p.status === 'ACTIVE' ? 'success' : 'default'} className="rounded-full px-2.5">
                      {p.status}
                    </Tag>
                  </div>

                  <h3 className="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors mb-1.5">
                    {p.name}
                  </h3>
      ...
      <Modal
        title={<span className="font-bold text-gray-800 text-lg">Tạo dự án mới</span>}
      ...
      >
        ...
        >
          <Form.Item
            label={<span className="text-gray-700 text-sm font-semibold">Tên dự án</span>}
            name="name"
          ...
          <Form.Item
            label={<span className="text-gray-700 text-sm font-semibold">Mã dự án (Key)</span>}
            name="key"
            tooltip="Mã định danh viết tắt (2-10 ký tự), dùng làm slug URL (ví dụ: TASK, WEB)."
          >
            <Input placeholder="Tự động tạo nếu để trống (ví dụ: TASK)" maxLength={10} className="uppercase" />
          </Form.Item>

          <Form.Item
            label={<span className="text-gray-700 text-sm font-semibold">Mô tả</span>}
            name="description"
          >
            <Input.TextArea placeholder="Mô tả mục tiêu của dự án..." rows={3} />
          </Form.Item>
