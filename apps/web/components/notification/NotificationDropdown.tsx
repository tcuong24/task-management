...
import { MailOutlined, CarryOutOutlined, BellOutlined, CheckCircleOutlined } from '@ant-design/icons';
...
export default function NotificationDropdown({
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'ORG_INVITE':
        return <Avatar icon={<MailOutlined />} className="bg-indigo-100 text-indigo-600" />;
      case 'TASK_ASSIGNED':
        return <Avatar icon={<CarryOutOutlined />} className="bg-blue-100 text-blue-600" />;
      default:
        return <Avatar icon={<BellOutlined />} className="bg-gray-100 text-gray-600" />;
    }
  };
  ...
  return (
    <div className="w-[360px] bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100">
      ...
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
        <span className="font-semibold text-gray-800 text-sm">Thông báo gần đây</span>
              ...
              >
                ...
                <div className="flex-1 min-w-0 pr-4">
                  <div className="flex items-start justify-between gap-1 mb-1">
                    <span className={`text-xs text-gray-800 truncate block ${!item.isRead ? 'font-semibold' : ''}`}>
                      {item.title}
                    </span>
                    {!item.isRead && (
                  ...
                  <p className="text-xs text-gray-500 line-clamp-2 mb-1">{item.content}</p>
