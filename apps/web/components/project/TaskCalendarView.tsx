          ...
          <div className="grid grid-cols-7 auto-rows-fr divide-x divide-y divide-gray-200/70">
            {calendarGrid.map((date) => {
              ...
              const hasOverdueUndone = dayTasks.some(
                (t) =>
                  dayjs(dateStr).isBefore(dayjs(), "day") &&
                  t.status !== "DONE",
              );
              ...
              return (
                <div
                  ...
                  } ${hasOverdueUndone ? "bg-red-50/40 border-red-200/60" : ""} hover:bg-gray-50/30`}
                >
                    ...
                    {visibleTasks.map((t) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-1 px-1.5 rounded-lg border text-[11px] font-medium leading-tight truncate transition-colors ${
                            isDone
                              ? "bg-slate-100 border-slate-200 text-gray-400 line-through"
                              : "bg-white border-gray-200/90 text-gray-800 shadow-2xs hover:border-gray-300 hover:text-gray-700"
                          }`}
                          title={`${code}: ${t.title}`}
                        >
                          <span className="font-mono font-bold mr-1 text-gray-700">
                            {isDone && (
                              <CheckCircleFilled className="text-emerald-500 text-[10px] mr-1" />
                            )}
                            {code}
                          </span>
                          <span>{t.title}</span>
                    ...
                    {remainingCount > 0 && (
                        ...
                        content={
                          <div className="flex flex-col gap-1.5 min-w-[240px] max-h-64 overflow-y-auto p-1">
                            <span className="text-xs font-bold text-gray-500 pb-1 border-b border-gray-100">
                              Công việc ngày {date.format("DD/MM/YYYY")} (
                              {dayTasks.length})
                            </span>
                            {dayTasks.map((t) => {
                              const isDone = t.status === "DONE";
                              ...
                              return (
                                <div
                                  key={t.id}
                                  onClick={(e) => handleTaskClick(t, e)}
                                  className="flex items-center justify-between p-1.5 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-100 cursor-pointer text-xs transition-colors"
                                >
                                  <div className="flex items-center gap-1.5 truncate">
                                    ...
                                    <span
                                      className={`truncate ${isDone ? "line-through text-gray-400" : "text-gray-800"}`}
                                    >
                                      {t.title}
                                    </span>
                                  </div>
                                  <Tag
                                    className={`m-0 text-[9px] border-none px-1.5 py-0.2 rounded font-semibold ${stBadge.bg} ${stBadge.text}`}
                                  >
                                    {stBadge.label}
                                  </Tag>
                                </div>
                      ...
                      >
                        <button
                          ...
                          className="text-[10px] font-bold text-gray-700 hover:text-gray-900 bg-gray-50/70 hover:bg-gray-100 px-1.5 py-0.5 rounded-md text-left transition-colors cursor-pointer border border-gray-100"
                        >
                          +{remainingCount} more...
                        </button>
                      </Popover>
