...
export function UpcomingDeadlines({ orgSlug, upcomingDeadlines = [] }: UpcomingDeadlinesProps) {
      ...
      {upcomingDeadlines.length === 0 ? (
        ...
        <div className="flex flex-col divide-y divide-gray-100">
          {upcomingDeadlines.map((t) => {
            ...
            const prio = PRIORITY_BADGES[t.priority] || { label: t.priority, color: 'default' };
              ...
              >
                <div className="py-2.5 first:pt-1 last:pb-1 flex items-center justify-between gap-3 hover:bg-slate-50/70 rounded-xl px-2 -mx-2 transition-colors cursor-pointer group">
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    ...
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-bold text-gray-900 truncate group-hover:text-indigo-600">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-gray-400 mt-0.5">
                        <span className="flex items-center gap-1">
                          <Avatar
                            ...
                            icon={!t.assigneeAvatarUrl ? <UserOutlined /> : undefined}
                            className="bg-gray-200 text-gray-600 text-[9px]"
                          >
                            {t.assigneeName.charAt(0).toUpperCase()}
                          </Avatar>
                          <span className="font-medium text-gray-600">{t.assigneeName}</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <RightOutlined className="text-[10px] text-gray-300 group-hover:text-indigo-600" />
                  </div>
                </div>
              </Link>
            );
