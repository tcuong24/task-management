...
export function formatActivity(
  ...
  const displayCode =
    meta.taskNumber && projectKey ? `[${projectKey}-${meta.taskNumber}] ` : "";
    ...
    if (orgSlug && projectKey && log.entityId) {
      return (
        <Link
          ...
          className="font-bold text-gray-700 hover:text-gray-900 underline hover:no-underline transition-colors mx-0.5"
        >
          {label}
        </Link>
      );
  ...
  const renderProjectLink = (name: string) => {
    ...
    if (
      orgSlug &&
      (projectKey || (log.entityType === "PROJECT" && log.entityId))
    ) {
      ...
      return (
        <span>
          ...
          <Link
            ...
            className="font-bold text-gray-700 hover:text-gray-900 underline hover:no-underline transition-colors"
          >
            "{name}"
          </Link>
        </span>
    ...
    return (
      <span>
        {" "}
        trong dự án <span className="font-bold text-gray-900">"{name}"</span>
      </span>
    );
  };
  ...
  switch (log.action) {
    ...
    case "status_changed": {
      ...
      return (
        <span>
          ...
          {renderProjectLink(projectName)} từ{" "}
          <span className="font-semibold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded text-xs border border-gray-200">
            "{oldSt}"
          </span>{" "}
          sang{" "}
          <span className="font-semibold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded text-xs border border-gray-200">
    ...
    case "assignee_changed": {
      const label = taskTitle ? `"${displayCode}${taskTitle}"` : "công việc";
      const assignee =
        (meta.assigneeName as string) ?? log.newValue ?? "Chưa phân công";
      return (
        <span>
          đã gán công việc {renderTaskLink(label)}
          {renderProjectLink(projectName)} cho{" "}
    ...
    case "priority_changed": {
      ...
      return (
        <span>
          ...
          {renderProjectLink(projectName)} sang{" "}
          <span className="font-bold text-gray-900">"{log.newValue}"</span>
        </span>
      );
    }
    ...
    case "title_changed": {
      return (
        <span>
          đã đổi tên công việc từ{" "}
          <span className="font-bold text-gray-900">"{log.oldValue}"</span> sang{" "}
          {renderTaskLink(`"${log.newValue}"`)}
          {renderProjectLink(projectName)}
        </span>
      );
    }
    ...
    case "role_changed":
      return (
        <span>
          đã đổi vai trò của{" "}
          <span className="font-bold text-gray-900">
            {(meta.memberName as string) ?? "thành viên"}
          </span>{" "}
          thành <span className="font-bold text-gray-900">{log.newValue}</span>
        </span>
      );
